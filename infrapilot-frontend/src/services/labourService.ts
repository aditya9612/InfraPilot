...
export const labourService = {
     ...
     */
    async getLabourDashboard(): Promise<{
        user_name: string;
        ...
        try {
            console.log("GET /api/v1/dashboard/labour");
            const response = await api.get<any>("/dashboard/labour");
            const data = response.data?.data || response.data;
            console.log("GET /api/v1/dashboard/labour - SUCCESS", data);
            return data;
        } catch (error: any) {
            console.warn("getLabourDashboard API error, using fallback:", error.message);
            const userStr = localStorage.getItem("infrapilot_user");
            if (userStr) {
                try {
                    const u = JSON.parse(userStr);
                    if (u.mobile === "8080808080" || u.mobile === "7575757575") {
                        console.log("getLabourDashboard fallback - Mock user detected, returning mock dashboard data");
                        return {
                            user_name: u.mobile === "7575757575" ? "Rama Sharma" : "Gopal Yadav",
                            project_name: "City Heights Residency",
                            ...
                            recent_tasks: [
                                { id: "t1", title: "Foundation Work - Block A", project_name: "City Heights Residency", status: "In Progress" },
                                { id: "t2", title: "Plastering - 3rd Floor", project_name: "City Heights Residency", status: "Completed" },
                                { id: "t3", title: "Tile Laying - Lobby", project_name: "City Heights Residency", status: "Pending" },
                            ],
                            recent_activity: [
                                { id: 1, type: "check_in", description: "Checked in at site", timestamp: new Date(Date.now() - 2 * 3600000).toISOString(), location: "City Heights Residency" },
                                { id: 2, type: "task_completed", description: "Completed: Plastering - 3rd Floor", timestamp: new Date(Date.now() - 5 * 3600000).toISOString() },
                                { id: 3, type: "payment", description: "Payment received ₹6,250", timestamp: new Date(Date.now() - 2 * 86400000).toISOString() },
                                { id: 4, type: "check_out", description: "Checked out from site", timestamp: new Date(Date.now() - 86400000).toISOString(), location: "City Heights Residency" },
                                { id: 5, type: "attendance", description: "Attendance marked - Present", timestamp: new Date(Date.now() - 3 * 86400000).toISOString() },
                            ]
                        };
                    }
                } catch (e) {
                    console.error("Failed to parse user from localStorage in getLabourDashboard fallback", e);
                }
            }
            return {
                user_name: "Labour Worker",
                project_name: null,
