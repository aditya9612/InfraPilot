...
const ManagerAttendancePage: React.FC = () => {
    const { user } = useAuth();
    const { selectedProjectId: activeProjectId = 0 } = useProject();
    ...
    const fetchData = async (silent = false) => {
        ...
        try {
            ...
            const rawItems = Array.isArray(list) ? list : ((list as any)?.data || (list as any)?.items || []);
    ...
    return (
        <>
            <Navbar
                title="My Attendance"
                breadcrumb={['Manager', 'My Attendance']}
            />
                            ...
                            <div className="grid grid-cols-2 gap-y-4 gap-x-4">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-xs font-black text-slate-700">
                                        {(selectedRecordForDetail as any).contractor_name || (selectedRecordForDetail as any).contractor || (user as any)?.contractor_name || (user as any)?.contractor || "—"}
                                    </p>
                                </div>
                                <div className="space-y-1">
