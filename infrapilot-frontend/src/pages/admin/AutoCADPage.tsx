  ...
  while (i < lines.length) {
    ...
    if (code === "0") {
      ...
      if (
        hasX &&
        hasY &&
        (currentEntity === "POINT" || currentEntity === "VERTEX")
      ) {
        points.push({ x, y });
        hasX = false;
        hasY = false;
      ...
      ["POINT", "VERTEX", "LINE", "LWPOLYLINE"].includes(currentEntity)
    ) {
      if (code === "10") {
        x = parseFloat(value);
        hasX = true;
...
function renderToCanvas(
  ...
  const toCanvasY = (y: number) =>
    MARGIN.top + plotH - ((y - minY) / rangeY) * plotH;
...
export default function AutoCADPage() {
  ...
  const [chartTitle, setChartTitle] = useState(
    "AutoCAD Point Visualization (Top View)",
  );
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (points.length === 0) {
            setError(
              "Could not parse coordinate data. Ensure your CSV has 'Easting'/'X' and 'Northing'/'Y' columns.",
            );
            return;
          }
  ...
  // ── Step 2: Upload DXF ─────────────────────────────────────────────────────
  const handleDXFUpload = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setError(null);
      const file = e.target.files?.[0];
      if (!file) return;
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (pts.length === 0) {
            setError(
              "No coordinate entities found in the DXF file. Supported: POINT, LINE, LWPOLYLINE.",
            );
            return;
          }
          setParsedPoints(pts);
          ...
          setChartTitle(
            `${file.name.replace(".dxf", "")} - Point Visualization (Top View)`,
          );
          setStep("view");
        } catch {
  ...
  const useConvertedPoints = () => {
    ...
    setChartTitle(
      `${convertedFileName.replace(/\..+$/, "")} - Point Visualization (Top View)`,
    );
    setStep("view");
  };

  // ── Backend Convert API (CSV to DXF) ────────────────────────────────────
  const handleBackendConvert = async () => {
    ...
    const toastId = toast.loading("Converting CSV to DXF...");
    ...
    try {
      ...
      link.href = url;
      const downloadName = selectedFile.name.replace(/\.[^/.]+$/, "") + ".dxf";
      ...
      toast.success(`Successfully converted to ${downloadName}`, { id: toastId });
    } catch (err) {
      console.error("Conversion error:", err);
      toast.error("Failed to convert file. Ensure format is correct.", { id: toastId });
    } finally {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-slate-500 text-sm">
              Convert, upload and visualize AutoCAD DXF drawings as scatter
              plots.
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-0">
            {steps.map((s, idx) => (
              <div key={s.id} className="flex items-center flex-1">
                <button
                  onClick={() =>
                    s.id === "view" && parsedPoints.length === 0
                      ? null
                ...
                >
                  ...
                  <span
                    className={`text-sm font-semibold hidden sm:block ${step === s.id ? "text-primary" : "text-slate-400"}`}
                  >
                    {s.label}
                  </span>
                </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
              ...
              <p className="text-sm text-slate-500 mb-5">
                Upload a CSV or TXT file containing coordinate data (Easting/X
                and Northing/Y columns).
              </p>
              ...
              >
                ...
                <p className="font-semibold text-slate-700 text-sm">
                  Click to select CSV / TXT file
                </p>
                <p className="text-xs text-slate-400 mt-1">
                  Must have Easting/X and Northing/Y columns
                </p>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex flex-col">
              ...
              <p className="text-sm text-slate-500 mb-5">
                Preview and proceed to visualization.
              </p>
              ...
              {convertedPoints.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-slate-300">
                  ...
                  <p className="text-sm">
                    No data yet. Upload a file to convert.
                  </p>
                </div>
              ) : !hasConverted ? (
                /* Ready to Convert State */
                <div className="flex-1 flex flex-col items-center justify-center p-8 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
                  ...
                  <p className="text-sm text-slate-500 text-center mb-8 max-w-[280px]">
                    ...
                    <span className="font-bold text-slate-700">
                      {convertedPoints.length.toLocaleString()}
                    </span>{" "}
                    points from{" "}
                    <span className="italic">{convertedFileName}</span>.
                  </p>

                  <button
                    onClick={handleBackendConvert}
                    disabled={isConverting}
                    className="w-full max-w-[240px] py-4 bg-primary text-white font-bold rounded-2xl shadow-xl shadow-primary/20 hover:bg-blue-600 transition-all flex items-center justify-center gap-3 text-lg disabled:opacity-50"
                  >
                    {isConverting ? (
                      <>
                ...
                <div className="flex-1 flex flex-col gap-4">
                  <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-emerald-600 text-xs">
                        {convertedPoints.length.toLocaleString()} coordinate
                        points extracted
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="overflow-y-auto overflow-x-auto flex-1">
                      <table className="w-full text-xs min-w-full">
                        <thead className="bg-slate-100 text-slate-500 font-bold uppercase tracking-wider sticky top-0 z-10">
                          <tr>
                            <th className="px-4 py-2.5 text-left bg-slate-100">
                              #
                            </th>
                            {csvHeaders.map((h, i) => (
                              <th
                                key={i}
                                className="px-4 py-2.5 text-left whitespace-nowrap bg-slate-100"
                              >
                                {h}
                              </th>
                            ))}
                        ...
                        <tbody className="divide-y divide-slate-100 bg-white">
                          {convertedPoints.slice(0, 100).map((p, i) => (
                            ...
                            >
                              <td className="px-4 py-2 text-slate-400 font-mono border-r border-slate-100/50">
                                {i + 1}
                              </td>
                              {p.rawRow?.map((val, idx) => (
                                <td
                                  key={idx}
                                  className="px-4 py-2 whitespace-nowrap overflow-hidden text-ellipsis max-w-[200px]"
                                  title={val}
                                >
                                  {val}
                                </td>
                    ...
                    {convertedPoints.length > 100 && (
                      <div className="px-4 py-2 bg-slate-100/50 border-t border-slate-100 text-center">
                        <p className="text-[10px] text-slate-400 font-medium italic">
                          ...
                          {convertedPoints.length.toLocaleString()} points
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-auto">
                    <button
                      ...
                      className="flex-1 min-w-[140px] py-2.5 border border-slate-200 text-slate-600 font-semibold rounded-xl hover:bg-slate-50 text-sm transition-all"
                    >
                      Upload DXF Instead
                    </button>
                    <button
                      onClick={useConvertedPoints}
                      className="flex-1 min-w-[140px] py-2.5 bg-primary text-white font-bold rounded-xl shadow-md shadow-primary/20 hover:bg-blue-600 text-sm transition-all"
                    >
                      Visualize →
                    </button>
                  </div>
        ...
        {step === "upload" && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 max-w-2xl mx-auto">
            ...
            <p className="text-sm text-slate-500 mb-6">
              Upload an ASCII DXF file exported from AutoCAD. The system will
              extract all POINT, LINE and POLYLINE entities.
            </p>
            ...
            >
              ...
              <p className="text-xs text-slate-400 mt-1">
                ASCII DXF format only. No size limit.
              </p>
            </div>
            ...
            {dxfFileName && (
              <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center gap-3 mb-4">
                ...
                <p className="text-sm font-semibold text-emerald-700">
                  {dxfFileName} loaded with{" "}
                  {parsedPoints.length.toLocaleString()} points
                </p>
              </div>
            )}
            ...
            <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-400">
                  {parsedPoints.length.toLocaleString()} coordinate points
                  rendered
                </p>
              </div>
              ...
              {isLoading && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-slate-50/90 backdrop-blur-sm rounded-b-2xl">
                  <div className="relative flex items-center justify-center mb-5">
                    ...
                    {/* Inner ring */}
                    <div
                      className="absolute w-10 h-10 rounded-full border-4 border-blue-300/30 border-b-blue-400 animate-spin"
                      style={{
                  ...
                  <p className="text-xs text-slate-400">
                    {parsedPoints.length.toLocaleString()} points being plotted
                  </p>
                  {/* Progress bar */}
            ...
            <div className="px-6 py-3 border-t border-slate-100 bg-slate-50/50 flex flex-wrap items-center gap-6 text-xs font-medium text-slate-500">
              <span>
                ...
                <strong className="text-slate-700">
                  {parsedPoints.length.toLocaleString()}
                </strong>
              </span>
              <span>
                Min X:{" "}
                <strong className="text-slate-700">
                  {Math.min(...parsedPoints.map((p) => p.x)).toFixed(2)}
                </strong>
              </span>
              <span>
                Max X:{" "}
                <strong className="text-slate-700">
                  {Math.max(...parsedPoints.map((p) => p.x)).toFixed(2)}
                </strong>
              </span>
              <span>
                Min Y:{" "}
                <strong className="text-slate-700">
                  {Math.min(...parsedPoints.map((p) => p.y)).toFixed(2)}
                </strong>
              </span>
              <span>
                Max Y:{" "}
                <strong className="text-slate-700">
                  {Math.max(...parsedPoints.map((p) => p.y)).toFixed(2)}
                </strong>
              </span>
            </div>
          </div>
