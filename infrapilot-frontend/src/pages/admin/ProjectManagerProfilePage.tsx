...
import { ChevronLeft, Mail, Phone, MapPin, Calendar, CreditCard, Fingerprint, LayoutGrid, LayoutList, AlertTriangle, Users, TrendingUp, Camera } from "lucide-react";
...
const ProjectManagerProfilePage: React.FC = () => {
    const { id } = useParams();
    const navigate = useNavigate();
    const [mirrorFilter, setMirrorFilter] = useState<"action" | "team" | "portfolio" | "photos">("action");
        ...
        const resolveProjects = async () => {
            ...
            try {
                ...
                if (targetProjects.length > 0) {
                    console.log(`Portfolio mode initialized.`);
                    setActiveProjectId(null); // 'null' means All Projects (Portfolio view)
                } else {
                    console.warn(`No projects resolved for PM ${u.user_id}`);
                    setIsLoading(false);
                }
            } catch (error) {
    ...
    useEffect(() => {
        const fetchProjectData = async () => {
            ...
            try {
                ...
                const projectsToFetch = activeProjectId ? [activeProjectId] : assignedProjects.map(p => p.id);
                ...
                await Promise.all(projectsToFetch.map(async (pid) => {
                    ...
                    allDsrs = allDsrs.concat((dsrsRes as any)?.items || []);
                    allPhotos = allPhotos.concat((photos as any)?.items || []);
                    allMaterialLogs = allMaterialLogs.concat(Array.isArray(mlRes) ? mlRes : []);
                ...
                const latestDsr = [...allDsrs].sort((a, b) => new Date(b.report_date).getTime() - new Date(a.report_date).getTime())[0];
                ...
                if (allAttendance.length > 0) {
                    totalCount = allAttendance.length;
                    skilledCount = allAttendance.filter((l: any) => normalizeSkill(l).includes("skilled") && !normalizeSkill(l).includes("unskilled")).length;
                    unskilledCount = totalCount - skilledCount;
                } else {
                    // Gather generic registration details if no attendance
                    await Promise.all(projectsToFetch.map(async (pid) => {
                        const regDetails = await labourService.getLabours(pid, { limit: 100 }).catch(() => ({ items: [] }));
                ...
                setIssueAnalytics(allIssueAnalytics);
                setVitals({
                    total_labour_today: totalCount,
                    skilled_labour: skilledCount,
                    unskilled_labour: unskilledCount,
                    active_activities: currentActivities.length,
                ...
                setDsrData([...allDsrs].sort((a, b) => new Date(b.report_date).getTime() - new Date(a.report_date).getTime()));
                setDsrPage(0);
                setSitePhotos([...allPhotos].sort((a, b) => new Date(b.created_at || b.date).getTime() - new Date(a.created_at || a.date).getTime()));

                const u = engineerData;
                const activeProject = activeProjectId ? assignedProjects.find(p => p.id === activeProjectId) : null;
                const activeTask = currentActivities[0]?.activity_name || allDsrs[0]?.work_done?.split('.')[0] || "Portfolio Supervision";
                ...
                setEngineer({
                    ...
                    activeProjectName: activeProject?.project_name || "Portfolio Overview",
                    ...
                    status: u.is_active ? "Active" : "Leave",
                    specialization: u.designation || "Project Manager",
    ...
    if (!engineer) {
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-slate-50">
                ...
                <button onClick={() => navigate("/admin/managers")} className="px-4 py-2 bg-primary text-white rounded-xl text-sm font-bold">Back to Staff</button>
            </div>
        );
    }
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            try {
                ...
                doc.text("PROJECT MANAGER REPORT", 14, 16);
                doc.setFontSize(9);
                ...
                doc.setFontSize(12);
                doc.setFont("helvetica", "bold");
                doc.text("Project Manager Profile", 14, 50);
    ...
    return (
        <>
            <Navbar title="Project Manager Intelligence" breadcrumb={["Admin", "Project Managers", engineer.name]} />
                ...
                <div className="bg-gradient-to-r from-indigo-900 via-blue-900 to-slate-900 px-6 md:px-10 pt-8 pb-0 relative overflow-hidden -mt-20 pt-[calc(5rem+2rem)]">
                    ...
                    <div className="relative z-10">
                        <button onClick={() => navigate("/admin/managers")}
                            ...
                            <ChevronLeft className="w-4 h-4" /> Back to Project Managers
                        </button>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-white/10 rounded-t-2xl overflow-hidden">
                            {[
                                { label: "Assigned Projects", value: assignedProjects.length, icon: <LayoutGrid className="w-4 h-4" />, color: "text-white" },
                                { label: "Pending Approvals", value: dsrData.filter(d => (d.status || '').toLowerCase() === "submitted").length, icon: <AlertTriangle className="w-4 h-4" />, color: "text-rose-300" },
                                { label: "Est. Team Size", value: assignedProjects.length * 2, icon: <Users className="w-4 h-4" />, color: "text-blue-200" },
                                { label: "Portfolio Progress", value: `${vitals.progress}%`, icon: <TrendingUp className="w-4 h-4" />, color: "text-emerald-300" },
                            ].map(s => (
                ...
                <div className="p-6 md:p-10 grid grid-cols-1 xl:grid-cols-[300px_1fr] gap-8 items-start bg-slate-100">
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="bg-slate-900 rounded-2xl p-5 text-white relative overflow-hidden">
                            <div className="absolute bottom-0 right-0 w-28 h-28 bg-emerald-500/20 rounded-full blur-2xl -mb-14 -mr-14" />
                            <h3 className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-4">Portfolio Financials</h3>
                            <div className="grid grid-cols-2 gap-4 relative z-10 mb-4">
                                <div>
                                    <div className="flex items-center gap-1.5 text-emerald-400 mb-1">
                                        <CreditCard className="w-3.5 h-3.5" />
                                        <span className="text-[9px] font-black uppercase tracking-widest">Disbursed</span>
                                    </div>
                                    <p className="text-xl font-black">₹{vitals.total_expenses?.toLocaleString("en-IN") || 0}</p>
                                </div>
                                <div>
                            ...
                            <div className="pt-4 border-t border-white/10 flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs font-bold text-slate-300">Budget Health: <span className="text-emerald-400">Stable</span></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <h3 className="text-sm font-black text-slate-800">Team Compliance</h3>
                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Active Members: {assignedProjects.length * 2}</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="p-3 bg-emerald-50 rounded-xl text-center">
                                    <p className="text-2xl font-black text-emerald-600">{dsrData.filter((d) => (d.status || '').toLowerCase() === 'approved').length}</p>
                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Approved Reports</p>
                                </div>
                                <div className="p-3 bg-rose-50 rounded-xl text-center">
                                    <p className="text-2xl font-black text-rose-600">{dsrData.filter((d) => (d.status || '').toLowerCase() === 'rejected').length}</p>
                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Rejected Reports</p>
                                </div>
                            </div>
                        </div>
                        {/* Issue Analytics */}
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col overflow-hidden" style={{ height: 'calc(100vh - 7rem)', position: 'sticky', top: '1rem' }}>
                        ...
                        <div className="px-6 pt-5 pb-0 flex items-center justify-between border-b border-slate-100 flex-wrap gap-3">
                            <div className="flex gap-1">
                                {[
                                    { key: "action", label: "Action Required", count: dsrData.filter(d => (d.status || '').toLowerCase() === "submitted").length },
                                    { key: "team", label: "My Team", count: assignedProjects.length * 2 },
                                    { key: "portfolio", label: "Portfolio Reports", count: dsrData.length },
                            ...
                            {assignedProjects.length > 0 && (
                                <div className="flex items-center gap-2 pb-2">
                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest hidden md:inline">Active Site:</span>
                                    <select className="bg-slate-50 border border-slate-200 text-xs font-bold text-primary rounded-lg px-3 py-1.5 outline-none cursor-pointer hover:bg-slate-100"
                                        value={activeProjectId || ""} onChange={(e) => setActiveProjectId(e.target.value ? Number(e.target.value) : null)}>
                        ...
                        <div className="flex-1 overflow-hidden p-6 flex flex-col min-h-0">
                            ...
                            {mirrorFilter === "action" && (() => {
                                const pendingDsrs = dsrData.filter(d => (d.status || '').toLowerCase() === 'submitted');
                                const pagedDsrs = pendingDsrs.slice(dsrPage * DSR_PAGE_SIZE, (dsrPage + 1) * DSR_PAGE_SIZE);
                                return (
                                    <div className="flex flex-col h-full">
                                        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                                            {pendingDsrs.length > 0 ? pagedDsrs.map((dsr, i) => (
                                                <div key={dsr.id || i} className="bg-slate-50 border border-slate-100 rounded-xl p-4 flex items-center justify-between hover:bg-white transition-all">
                                                    <div>
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <span className="w-2 h-2 rounded-full bg-rose-500" />
                                                            <span className="text-[10px] font-black text-rose-500 uppercase tracking-widest">Pending Review</span>
                                                        </div>
                                                        <p className="text-sm font-bold text-slate-800">{dsr.work_done?.split('.')[0] || "DSR Submission"}</p>
                                                        <p className="text-xs text-slate-500">{new Date(dsr.report_date).toLocaleDateString()}</p>
                                                    </div>
                                                    <button onClick={() => navigate('/admin/approvals')} className="px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg text-xs font-bold shadow-sm hover:text-primary hover:border-primary transition-all">Review</button>
                                                </div>
                                            )) : (
                                ...
                                <div className="flex flex-col h-full">
                                    <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                                        {teamMembers.length > 0 ? teamMembers.map((m, i) => (
                                            <div key={m.id || i} className="bg-slate-50 border border-slate-100 rounded-xl p-4 flex items-center justify-between hover:bg-white transition-all">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-bold text-slate-800">
                                                            {m.user?.full_name || m.user?.name || m.full_name || m.name || m.username || "Unknown Staff"}
                                                        </p>
                                                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                                                            {m.role?.name || m.role || m.user?.role?.name || m.user?.role || "Site Engineer"}
                                                        </p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    {(m.user?.mobile_number || m.mobile_number) && <a href={`tel:${m.user?.mobile_number || m.mobile_number}`} title="Call" className="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-600 hover:text-primary hover:border-primary transition-all"><Phone className="w-3.5 h-3.5" /></a>}
                                                    {(m.user?.email || m.email) && <a href={`mailto:${m.user?.email || m.email}`} title="Email" className="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-600 hover:text-primary hover:border-primary transition-all"><Mail className="w-3.5 h-3.5" /></a>}
                                                </div>
                                            </div>
                                        )) : (
                                            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                                                <Users className="w-8 h-8 mb-3 opacity-30" />
                                                <p className="font-bold uppercase tracking-widest text-xs">No active staff found</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
