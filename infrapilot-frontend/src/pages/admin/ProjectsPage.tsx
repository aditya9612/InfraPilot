...
const ProjectsPage = () => {
  ...
  const handleViewProject = (id: number, tab?: string) => {
    ...
    const basePath = window.location.pathname.startsWith("/admin")
      ? "/admin"
      : "/manager";
    const path = `${basePath}/projects/${id}${tab ? `?tab=${tab}` : ""}`;
    navigate(path);
  };
                    ...
                    {filtered
                      ...
                      .map((p) => (
                        <div
                          ...
                          className="group cursor-pointer bg-slate-50/50 rounded-xl p-4 hover:bg-slate-50 transition-all border border-slate-50 hover:border-primary/20 hover:shadow-md"
                        >
