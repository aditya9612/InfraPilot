...
const alertsData = [
  { id: 1, type: "Delay", message: "Excavation at Skyline Tower A is 3 days behind schedule.", target: "Project Manager, Admin", date: "2026-04-02", status: "Critical" },
  { id: 2, type: "Budget", message: "Material procurement for Metro Ph-II exceeded budget by 12%.", target: "Accountant, Admin", date: "2026-04-01", status: "Warning" },
  { id: 3, type: "Safety", message: "Weekly safety audit completed. 2 minor issues detected.", target: "Site Engineer", date: "2026-03-30", status: "Normal" },
];
      ...
      <PageTransition className="p-6 bg-slate-50 min-h-screen">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex gap-2">
            <button className="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-semibold hover:bg-slate-50 shadow-sm transition-all">Mark All Read</button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <StatCard title="Total Unread" value="12" sub="Across all projects" accent="text-primary" />
          <StatCard title="Critical Issues" value="3" sub="Requires immediate action" accent="text-rose-500" />
          <StatCard title="System Health" value="98%" sub="Uptime & delivery rate" accent="text-emerald-500" />
        </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredAlerts.map((alert) => (
                  <tr key={alert.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm text-slate-600 max-w-sm line-clamp-1 group-hover:line-clamp-none transition-all">{alert.message}</p>
                    </td>
