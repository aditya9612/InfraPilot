    ...
    <DashboardLayout>
      ...
      <PageTransition className="p-6 bg-slate-50 min-h-screen">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Client Portfolio</h1>
            <p className="text-slate-500 text-sm">Manage client relationships, project links, and financial history.</p>
          </div>
          <div className="flex gap-2">
            <button className="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-semibold hover:bg-slate-50 shadow-sm transition-all">Client Portal</button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <StatCard title="Total Clients" value="24" sub="5 Premium Accounts" accent="text-primary" />
          <StatCard title="Outstanding Billing" value="₹2.4Cr" sub="Across 8 Projects" accent="text-rose-500" />
          <StatCard title="Satisfaction Score" value="94%" sub="Based on project delivery" accent="text-emerald-500" />
        </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredClients.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs font-bold ${c.billing.includes("Pending") || c.billing.includes("Overdue") ? "text-rose-500" : "text-emerald-500"}`}>
                        {c.billing}
                      </span>
                    </td>
