          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
             <thead>
               <tr className="border-b border-slate-50">
                 <th className="py-6 text-[9px] font-black text-slate-400 uppercase tracking-widest">Request ID</th>
                 <th className="py-6 text-[9px] font-black text-slate-400 uppercase tracking-widest">Detail & Description</th>
                 <th className="py-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Type</th>
                 <th className="py-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Value / Qty</th>
                 <th className="py-6 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Signed On</th>
               </tr>
             </thead>
             <tbody className="divide-y divide-slate-50">
               {approvedItems.map((item, i) => (
                 <tr key={i} className="group hover:bg-slate-50/50 transition-colors">
                   <td className="py-8">
                      <p className="text-xs font-black text-slate-800 font-mono tracking-tight">{item.id}</p>
                   </td>
                   <td className="py-8 pr-10 max-w-lg">
                      <p className="text-sm font-black text-slate-700 mb-1">{item.description}</p>
                   ...
                   <td className="py-8 text-center">
                      <p className="text-sm font-black text-slate-800">{item.amountQuantity}</p>
                   </td>
                   <td className="py-8 whitespace-nowrap text-right pr-6">
                      <div className="flex items-center justify-end gap-2 text-emerald-600">
                         <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                         <div>
                            <p className="text-xs font-black">{item.approvedOn}</p>
