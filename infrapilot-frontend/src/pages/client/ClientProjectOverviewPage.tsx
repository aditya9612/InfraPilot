...
const ClientProjectOverviewPage = () => {
    ...
    return (
      <>
        ...
        <div className="flex flex-col items-center justify-center min-h-[80vh] bg-slate-50">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Accessing Project Specifications...</p>
        </div>
      </>
    );
  }
  ...
  const p = project || {
    project_name: "Skyline Tower - Phase 3 Extension",
    description: "Residential High-Rise (A+ Category)",
    start_date: "15 Oct 2025",
    end_date: "30 Sept 2026",
    status: "On Track",
    ...
    <>
      ...
      <div className="p-6 bg-slate-50 min-h-screen font-inter pb-12">
        <div className="mb-8">
          ...
          <p className="text-slate-400 font-medium mt-1 uppercase tracking-widest text-[10px]">{p.project_name} — Detailed Specification</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          <div className="lg:col-span-2 bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
              {[
                { label: "Project Name", value: p.project_name, icon: "🏢" },
                { label: "Location", value: "Worli, Mumbai South Central", icon: "📍" },
                { label: "Project Type", value: p.description, icon: "🏗️" },
                { label: "Total Budget", value: "₹22,20,00,000.00 (Incl. GST)", icon: "💰" },
                { label: "Start Date", value: p.start_date, icon: "📅" },
                { label: "End Date (EST)", value: p.end_date, icon: "🏁" },
                { label: "Project Status", value: p.status, icon: "🟢", status: p.status },
              ].map((item, i) => (
                <div key={i} className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className={`text-sm font-bold ${item.status === "On Track" ? "text-emerald-600" : item.status === "Delayed" ? "text-red-600" : "text-slate-800"}`}>{item.value}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div className="space-y-4">
                {[
                  { label: "Project Start", value: p.start_date },
                  { label: "Expected Handover", value: p.end_date },
