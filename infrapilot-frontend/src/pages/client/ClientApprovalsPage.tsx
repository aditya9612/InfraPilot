...
const ClientApprovalsPage = () => (
  <>
    ...
    <div className="p-6 bg-slate-50 min-h-screen font-inter pb-12">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Approvals</h1>
        <p className="text-slate-400 font-semibold mt-1 uppercase tracking-widest text-[10px]">Variation orders, design changes & approvals requiring your action</p>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 flex items-start gap-4 mb-8">
        ...
        <div>
          <p className="text-sm font-bold text-amber-800">1 Approval Pending Your Action</p>
          ...
          <button className="mt-3 px-5 py-2 bg-amber-500 text-white text-[10px] font-bold uppercase tracking-widest rounded-xl hover:bg-amber-600 transition-colors shadow-sm">Review Now</button>
        </div>
      </div>
        ...
        {approvals.map((apr, i) => (
          <div key={i} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-md transition-all">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{apr.id}</p>
                <h3 className="text-sm font-bold text-slate-800 mt-0.5">{apr.title}</h3>
                <div className="flex gap-4 mt-2">
                  ...
                  {apr.amount !== "—" && <p className="text-[10px] font-bold text-slate-700">Amount: {apr.amount}</p>}
                </div>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <span className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full ${statusStyle[apr.status]}`}>{apr.status}</span>
                {apr.status === "Pending Client" && (
                  <div className="flex gap-2">
                    <button className="px-4 py-2 bg-emerald-500 text-white text-[10px] font-bold rounded-xl hover:bg-emerald-600 transition-colors">Approve</button>
                    <button className="px-4 py-2 bg-red-50 text-red-600 text-[10px] font-bold rounded-xl hover:bg-red-100 transition-colors">Reject</button>
                  </div>
                )}
              </div>
            </div>
