                    ...
                    {!isLoading && filteredIssues.length > 0 && (
                        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50 sticky left-0 font-inter rounded-b-2xl">
                            ...
                            <div className="text-[11px] font-medium text-slate-500 hidden md:block">
                                Showing {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, filteredIssues.length)} of {filteredIssues.length} records
                            </div>
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <button
                                    onClick={() => setCurrentPage(prev => Math.min(Math.ceil(filteredIssues.length / itemsPerPage), prev + 1))}
                                    disabled={currentPage === Math.max(1, Math.ceil(filteredIssues.length / itemsPerPage)) || filteredIssues.length === 0}
                                    className="p-1.5 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-primary disabled:opacity-50 disabled:cursor-not-allowed transition-colors bg-white shadow-sm"
                                >
