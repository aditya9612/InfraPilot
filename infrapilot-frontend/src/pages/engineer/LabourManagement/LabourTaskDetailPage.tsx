...
const LabourTaskDetailPage = () => {
    ...
    useEffect(() => {
        if (activeTab === 'Labour Detail') {
            const fetchReports = async () => {
                try {
                    const wRes = await labourService.getLabourWeeklyReport(id || "1");
                    const mRes = await labourService.getLabourMonthlyReport(id || "1");
                    setWeeklyReport(Array.isArray(wRes) ? wRes[0] : wRes);
                    setMonthlyReport(Array.isArray(mRes) ? mRes[0] : mRes);
                } catch (error) {
    ...
    const filteredTasks = mockTasks.filter(task => {
        const matchesSearch = task.title.toLowerCase().includes(searchQuery.toLowerCase()) || task.assignedTo.name.toLowerCase().includes(searchQuery.toLowerCase());
