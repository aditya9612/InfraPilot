          ...
          {[
            ...
            <button key={t.path} onClick={() => navigate(t.path)}
              className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${
                tab === (t.path.includes("receipt") ? "receipt" : t.path.includes("consumption") ? "consumption" : "stock")
                  ...
                  {(["Store", "Site"] as const).map(loc => (
                    <button key={loc} type="button" onClick={() => setForm(f => ({ ...f, location: loc }))}
                      className={`flex-1 py-2.5 rounded-xl text-xs font-bold border transition-all ${
                        form.location === loc ? "bg-primary text-white border-primary" : "bg-slate-50 text-slate-500 border-slate-200"
                      }`}>
