...
const activity = [
  {
    id: 1,
    type: "invoice",
    text: "Invoice #INV-204 submitted",
    time: "2h ago",
    icon: "💰",
  },
  {
    id: 2,
    type: "boq",
    text: "BOQ updated for Steel items",
    time: "5h ago",
    icon: "📦",
  },
  {
    id: 3,
    type: "task",
    text: "Excavation completed for Block B",
    time: "1d ago",
    icon: "✔",
  },
];
          ...
          {[
            ...
            >
              <div>
                ...
                <p
                  className={`text-[10px] font-bold mt-1 ${card.sub.includes("+") ? "text-emerald-500" : "text-slate-400"}`}
                >
                  {card.sub}
                </p>
              </div>
              <span
                className={`w-12 h-12 ${card.color} rounded-2xl flex items-center justify-center text-xl shadow-inner`}
              >
                {card.icon}
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <h2 className="text-sm font-bold text-slate-800 uppercase tracking-widest text-[11px]">
                  Active Projects
                </h2>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                {projects.map((p) => (
                  <div
                    key={p.id}
                    className="p-5 bg-slate-50/50 border border-slate-100 rounded-2xl hover:border-blue-200 transition-colors group"
                  >
                    ...
                    <div className="mb-4">
                      ...
                      <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-1000 ${p.status === "On Track" ? "bg-primary" : "bg-red-500"}`}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button className="px-3 py-1.5 bg-slate-50 text-slate-600 text-[10px] font-bold rounded-lg hover:bg-slate-100">
                    DOWNLOAD CSV
                  </button>
                  <button className="px-3 py-1.5 bg-blue-50 text-primary text-[10px] font-bold rounded-lg hover:bg-blue-100">
                    EDIT BOQ
                  </button>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tfoot className="bg-blue-50/50 border-t-2 border-primary/10">
                    <tr>
                      <td
                        colSpan={4}
                        className="px-6 py-5 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest"
                      >
                        Grand Total
                      </td>
                      <td className="px-6 py-5 text-right text-lg font-black text-primary">
                        ₹{totalBOQ.toLocaleString()}
                      </td>
                    </tr>
                  </tfoot>
                </table>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
              <h2 className="text-sm font-bold text-slate-800 uppercase tracking-widest text-[11px] mb-6">
                Payment Overview
              </h2>
                ...
                {tasks.map((task) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p
                          className={`text-xs font-bold transition-all ${task.completed ? "text-slate-300 line-through" : "text-slate-700"}`}
                        >
                          {task.name}
                        </p>
                        <p className="text-[10px] text-slate-300 font-bold mt-1">
                          DUE: {task.deadline.toUpperCase()}
                        </p>
                      </div>
                    </div>
                    {!task.completed && (
            ...
            <div className="bg-slate-900 p-6 rounded-2xl shadow-lg">
              <h2 className="text-sm font-bold text-white uppercase tracking-widest text-[11px] mb-6">
                Recent Site Updates
              </h2>
              <div className="space-y-6 relative before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-800">
                {activity.map((act) => (
                  <div
                    key={act.id}
                    className="relative pl-8 flex flex-col items-start gap-1"
                  >
                    <span className="absolute left-0 top-0 w-5 h-5 rounded-full bg-slate-800 border-2 border-slate-900 flex items-center justify-center text-[10px]">
                      {act.icon}
                    </span>
            ...
            <div className="space-y-3">
              <div className="p-4 bg-red-100/50 border-l-4 border-red-500 rounded-lg flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-red-400 font-bold mt-0.5">
                    Skyline Tower A material costs +12%
                  </p>
                </div>
              </div>
              <div className="p-4 bg-amber-100/50 border-l-4 border-amber-500 rounded-lg flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-black text-amber-600 uppercase tracking-tighter">
                    Safety Audit Due
                  </p>
                  <p className="text-[10px] text-amber-400 font-bold mt-0.5">
                    Schedule for Metro Ext Ph-II on 2nd April
                  </p>
                </div>
              </div>
            </div>
