...
const ViewEngineerModal: React.FC<ViewEngineerModalProps> = ({
  ...
  const isExceptional =
    ...
    engineer.performance === "Outstanding";
  const reportSubmitted = engineer.reportStatus === "Submitted";
    ...
    >
      <div className="space-y-8 pb-4">
        ...
        <div className="relative overflow-hidden bg-primary rounded-2xl p-8 text-white shadow-2xl">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
            ...
            <div className="relative group">
              ...
              {/* Status indicator */}
              <div
                className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-full border-4 border-primary flex items-center justify-center ${
                  isOnSite ? "bg-emerald-500" : "bg-slate-400"
                }`}
              >
            ...
            <div className="text-center md:text-left space-y-2">
              ...
              <div className="pt-2 flex flex-wrap justify-center md:justify-start gap-3">
                <div className="flex items-center gap-1.5 px-3 py-1 bg-white/10 rounded-full border border-white/10">
                  <span className="text-[10px] font-bold text-white/60 uppercase tracking-tighter">Experience</span>
                  <span className="text-xs font-bold">{engineer.experience}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 px-2">
          ...
          >
            <InfoItem label="Experience" value={engineer.experience} />
            <InfoItem
              label="Performance Rating"
              value={engineer.performance}
              valueClass={isExceptional ? "text-primary" : "text-slate-700"}
            />
            <InfoItem
              label="Deployment Status"
              value={engineer.status}
              valueClass={isOnSite ? "text-emerald-600" : "text-slate-500"}
            />
          </Section>
          ...
          >
            <InfoItem label="Mobile" value={engineer.mobile} />
            <InfoItem label="Email" value={engineer.email} />
          </Section>

          {/* Assignments & Reports — full width */}
          <Section
            icon={
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
              <InfoItem label="Assigned Projects" value={engineer.projects || "Not assigned"} />
              <InfoItem
                label="Daily Report Status"
                value={engineer.reportStatus || "—"}
                valueClass={reportSubmitted ? "text-blue-600" : "text-amber-600"}
              />
            </div>
          </Section>
