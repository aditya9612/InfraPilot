...
const EditDSRModal = ({ isOpen, onClose, dsr, onSuccess }: EditDSRModalProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      // Intentionally empty or add any initialization logic needed
    }
  }, [isOpen]);
  ...
  const handleChange = (
    ...
    setFormData((prev) => ({ ...prev, [name]: name === "contractor_id" ? Number(value) : value }));
    if (errors[name]) {
      setErrors((prev) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!dsr) return;
    setIsLoading(true);
    const tid = toast.loading("Updating DSR...");
    try {
      await dsrService.updateDsr(dsr.id, formData);
      toast.success("DSR entry updated successfully!", { id: tid });
      onSuccess();
    } catch (error) {
      console.error("DSR update error:", error);
      toast.error("Failed to update DSR", { id: tid });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <form id="edit-dsr-form" onSubmit={handleSubmit} noValidate className="space-y-6">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input type="text" name="site_location" value={formData.site_location || ""} onChange={handleChange} placeholder="e.g. Tower A - Basement" className={inputClasses(errors.site_location)} />
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              <label className={labelClasses}>Work Done Today</label>
            ...
            <div>
              ...
              <textarea name="work_planned" value={formData.work_planned || ""} onChange={handleChange} rows={2} className={`${inputClasses(errors.work_planned)} resize-none`} />
            </div>
          </div>
        </div>
