        ...
        <div className="p-8 bg-slate-900 rounded-[2rem] text-white flex items-center gap-8 relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <h2 className="text-4xl font-black tracking-tight uppercase">INV-{String(invoice.id).padStart(3, '0')}</h2>
              <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border ${statusColors[invoice.status]}`}>
                {invoice.status}
              </span>
            </div>
            <p className="text-xs font-bold text-primary tracking-[0.2em] uppercase">{invoice.type} Invoice</p>
            <p className="text-sm font-medium text-slate-400">Issue Date: <span className="text-white font-bold">{new Date(invoice.created_at).toLocaleDateString('en-GB')}</span></p>
          </div>
        </div>
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-primary">
                ...
                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Bill To / Project Details</h3>
              </div>
              ...
              <div className="p-6 bg-slate-50/50 rounded-2xl border border-slate-100 space-y-1">
                ...
                <p className="text-lg font-black text-slate-800 tracking-tight">{project?.project_name || 'N/A'}</p>
                <p className="text-xs text-slate-400 font-medium">Wing A Construction - Premium Residential Complex</p>
              </div>

              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 ml-1">Reference ID</p>
                <p className="text-sm font-bold text-slate-700 ml-1">#REF-{invoice.reference_id}</p>
              </div>

              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 ml-1">Description</p>
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-primary">
                ...
                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Financial Breakdown</h3>
              </div>
              <div className="h-px bg-slate-100 w-full"></div>

              <div className="space-y-4 pt-2">
                <div className="flex justify-between items-center px-2">
                  <span className="text-sm font-bold text-slate-500">Base Amount</span>
                  <span className="text-sm font-black text-slate-800">₹{invoice.amount.toLocaleString('en-IN')}</span>
                </div>
                <div className="flex justify-between items-center px-2">
                  <span className="text-sm font-bold text-slate-500">GST ({invoice.gst_percent}%)</span>
                  <span className="text-sm font-black text-slate-800">₹{invoice.gst_amount.toLocaleString('en-IN')}</span>
                </div>
                <div className="flex justify-between items-center px-2">
                  <span className="text-sm font-bold text-slate-500">Tax ({invoice.tax_percent}%)</span>
                  <span className="text-sm font-black text-slate-800">₹{invoice.tax_amount.toLocaleString('en-IN')}</span>
                </div>
                ...
                <div className="pt-6 border-t border-slate-100 mt-4 flex justify-between items-center px-2">
                  ...
                  <span className="text-2xl font-black text-primary tracking-tighter">₹{invoice.total_amount.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
          </div>
