...
import type { CreateEquipmentRequest } from "../../services/equipmentService";
...
const NewEquipmentModal: React.FC<NewEquipmentModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  projectId,
  ...
  const handleChange = (
    ...
    setFormData((prev: CreateEquipmentRequest) => ({ ...prev, [name]: finalValue }));
  ...
  const validate = () => {
    ...
    if (formData.working_hours <= 0) errs.working_hours = "Working hours must be greater than 0";
    if (formData.fuel_used < 0) errs.fuel_used = "Fuel used cannot be negative";
    if (formData.rental_cost < 0) errs.rental_cost = "Rental cost cannot be negative";
  ...
  return (
    <Modal
      ...
      maxWidth="max-w-4xl"
    >
      <form id="equipment-form" onSubmit={handleSubmit} noValidate className="space-y-6">
        {/* Identity & Operator */}
