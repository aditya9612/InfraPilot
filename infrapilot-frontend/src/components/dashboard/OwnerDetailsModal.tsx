...
const OwnerDetailsModal: React.FC<OwnerDetailsModalProps> = ({ isOpen, onClose, owner }) => {
  ...
  const footer = (
    <button
      onClick={onClose}
      className="px-8 py-2.5 bg-primary text-white rounded-xl text-sm font-bold hover:bg-blue-600 transition-all shadow-lg shadow-primary/20"
    >
      Close
    </button>
  );
  ...
  return (
    <Modal
      ...
      maxWidth="max-w-3xl"
    >
      <div className="space-y-8 pb-4 font-inter">
        ...
        <div className="relative overflow-hidden bg-primary rounded-2xl p-8 text-white shadow-2xl">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
            <div className="relative group">
              ...
              <div className="absolute -bottom-2 -right-2 w-8 h-8 rounded-full border-4 border-primary bg-emerald-500 flex items-center justify-center">
            ...
            <div className="text-center md:text-left space-y-2">
              <div className="flex flex-col md:flex-row items-center gap-3">
                <h3 className="text-2xl font-black tracking-tight">{owner.name}</h3>
              ...
              <div className="pt-2 flex flex-wrap justify-center md:justify-start gap-3">
                <div className="flex items-center gap-1.5 px-3 py-1 bg-white/10 rounded-full border border-white/10">
                  <span className="text-[10px] font-bold text-white/60 uppercase tracking-tighter">Code</span>
                  <span className="text-xs font-bold">{owner.owner_code || 'N/A'}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 px-2">
          ...
          {/* Communication Hub */}
          <Section icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>} title="Communication Hub">
            <InfoItem label="Mobile Priority" value={owner.mobile} />
            <InfoItem label="Email Coordinate" value={owner.email} />
          </Section>

          {/* Document Verification */}
          <Section icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-7.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>} title="Verification Index" fullWidth>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
              <InfoItem label="Permanent Account Number (PAN)" value={owner.pan} valueClass="font-mono text-xs uppercase" />
              <InfoItem label="Operational Address" value={owner.address} />
            </div>
          </Section>
