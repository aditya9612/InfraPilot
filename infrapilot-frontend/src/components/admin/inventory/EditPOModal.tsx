...
interface EditPOModalProps {
    ...
    onClose: () => void;
    onSubmit: (id: number, data: { supplier_id: number; project_id: number; material_id: number; quantity: number; rate: number; boq_item_id?: number }) => Promise<void>;
    suppliers: Supplier[];
    projects: any[];
    inventory: Material[];
}
...
const EditPOModal: React.FC<EditPOModalProps> = ({
    ...
    useEffect(() => {
        if (po) {
            setFormData({
                ...
                material_id: po.material_id || 0,
                boq_item_id: po.boq_item_id || 0,
    ...
    useEffect(() => {
        if (formData.material_id !== 0 && filteredMaterials.length > 0 && po) {
            const isValid = filteredMaterials.some((m: any) => m.id === formData.material_id || m.material_id === formData.material_id);
            // If editing and material changes, ensure picking a valid material from the supplier. We don't want to reset if the current material is missing initially due to slow load.
        }
    }, [filteredMaterials, formData.material_id, po]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!payload.boq_item_id) {
                delete (payload as any).boq_item_id;
            }
            await onSubmit(po.id, payload);
            onClose();
        } finally {
            setLoading(false);
        }
    ...
    const selectedMaterial = inventory.find(m => m.id === formData.material_id || m.material_id === formData.material_id);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity font-inter">
            <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 bg-indigo-50/50">
                    <div>
                        <h2 className="text-xl font-black text-indigo-900 tracking-tight">Edit Purchase Order</h2>
                        <p className="text-xs text-indigo-400 mt-0.5 font-bold uppercase tracking-widest">
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-1.5">
                            ...
                            >
                                ...
                                {/* Make sure the current material is always shown if valid */}
                                {filteredMaterials.map((m: any) => (
                                    <option key={m.id || m.material_id} value={m.material_id || m.id}>
                                        {m.name || m.material_name}
                                    </option>
                                ))}
                                {/* If current material not in filtered somehow, still show it as an option so user can see it initially */}
                                {formData.material_id !== 0 && !filteredMaterials.some(m => m.id === formData.material_id || m.material_id === formData.material_id) && (
                                    <option value={formData.material_id} className="text-rose-500">
                                        Current Material (Invalid for Supplier/Site)
                                    </option>
                                )}
                            </select>
                        </div>
                    ...
                    {/* Summary */}
                    <div className="md:col-span-2 bg-slate-50 rounded-2xl p-5 flex justify-between items-center border border-slate-100 shadow-sm mt-2">
                    ...
                    <div className="mt-8 flex items-center justify-end gap-3 pt-6 border-t border-slate-100">
                        <button type="button" onClick={onClose} disabled={loading} className="flex-1 px-6 py-2.5 text-sm font-bold text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-xl transition-colors">Cancel</button>
                        <button
                            type="submit"
