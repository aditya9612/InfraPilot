...
const BankingDashboardWrapper = () => {
  return (
    <div className="space-y-6">
      <BankingKPIs title="BANKING DASHBOARD" />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={[
                ...
                { name: 'Feb', in: 150, out: 100 },
                { name: 'Mar', in: 180, out: 120 },
                { name: 'Apr', in: 140, out: 110 },
                { name: 'May', in: 210, out: 150 },
                { name: 'Jun', in: 250, out: 180 },
                ...
                <Bar dataKey="in" name="Money In" fill="#10b981" radius={[4, 4, 0, 0]} maxBarSize={40} />
                <Bar dataKey="out" name="Money Out" fill="#f43f5e" radius={[4, 4, 0, 0]} maxBarSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          ...
          <div className="space-y-4">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 transition-colors border border-slate-100 cursor-pointer">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${i % 2 === 0 ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}`}>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={i % 2 === 0 ? "M19 9l-7 7-7-7" : "M5 15l7-7 7 7"} /></svg>
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-slate-800">{i % 2 === 0 ? 'Client Payment' : 'Vendor Payment'}</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5">Today, 10:45 AM</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className={`text-sm font-bold ${i % 2 === 0 ? 'text-emerald-600' : 'text-slate-800'}`}>{i % 2 === 0 ? '+' : '-'}₹{25000 * i}</p>
                  <p className="text-[10px] text-slate-500 mt-0.5">HDFC Bank</p>
                </div>
              </div>
            ))}
          </div>
...
// --- MAIN PAGE ---
type TabKey = "dashboard" | "accounts" | "cash" | "bank-book" | "reconciliation";
